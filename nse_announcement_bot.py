...
def get_nifty_total_market_symbols():
  """Nifty Total Market की पूरी लिस्ट लोड करना"""
  index_urls = [
  ...
  for url in index_urls:
    try:
      ...
      print(f"URL fetch warning ({url}): {e}")

  if all_symbols:
    print(
        f"✅ Total Market Watchlist Loaded for Announcements:"
        f" {len(all_symbols)} Stocks Found!"
    )
    return all_symbols

  # Fallback लिस्ट ताकि कभी क्रैश न हो
  print("⚠️ ऑनलाइन लिस्ट लोड नहीं हो पाई, Fallback लिस्ट का उपयोग हो रहा है।")
  return {
      "RELIANCE",
      "TCS",
      "INFY",
...
# पूरी प्रक्रिया को try-except में रखा गया है ताकि एरर आने पर विंडो बंद न हो
try:
  print("🤖 बॉट इनिशियलाइज हो रहा है...")
    ...
    try:
      ...
      if response.status_code == 200:
        try:
          ...
          print("API Error: JSON में कन्वर्ट नहीं हो पाया।")
          return None
      else:
  ...
  print("🚀 Nifty Total Market Results & Announcements Bot शुरू हो गया है...")

  while True:
    t_str = datetime.now(IST).strftime("%d-%m-%Y | %H:%M:%S IST")
    print(
        f"[{t_str}] Nifty Total Market Corporate Announcements चेक किए जा रहे"
        " हैं..."
    )
    ...
    if announcements and isinstance(announcements, list):
      for item in announcements:
        ...
        if is_financial and ann_id not in sent_announcements:
          ...
          if len(sent_announcements) > 500:
            sent_announcements.pop()
  ...
  print("\n🛑 Bot को बंद कर दिया गया है।")
except Exception as e:
  print(f"\n❌ एक बड़ी एरर की वजह से बॉट बंद हुआ है:\n{e}")
